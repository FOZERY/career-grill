...
Банк содержит паттерны достижений, а не сведения об опыте пользователя. `X`, названия технологий, механизмы и результаты являются слотами для уже подтверждённой истории. Выбирай паттерн только когда проблема, личное действие, технология и способ измерения согласуются с активной главой. Не заполняй `X` правдоподобными числами и не добавляй технологию из банка без отдельного подтверждения; неподтверждённый вариант остаётся `реконструкцией для тренировки` и не попадает в `Паспорт для CV` как факт.
...
## 14 — Cloud AWS / GCP / Yandex Cloud

### Выбор платформы и bullets

При международной валютной удалёнке без более сильного сигнала сначала рассматривай AWS-буллеты этого раздела. Предложи 1–2 достижения, связанные с реальной инженерной проблемой: доступность и масштабирование, миграция на managed service, снижение cloud spend, IAM/security, IaC или наблюдаемость. Метрика описывает эффект — latency, availability, RTO/RPO, throughput, стоимость или сокращение ручной эксплуатации, — а не количество использованных AWS-сервисов.

Для российского рынка сначала следуй подтверждённой инфраструктуре. При self-hosted/on-premises варианте используй связанные разделы Kubernetes, CI/CD, IaC/GitOps, Observability и Reliability/SRE. При выбранном Yandex Cloud переноси только общий механизм подходящего cloud-кейса и подставляй реальный сервис, подтверждённый для компании и исторического периода; не переименовывай AWS-сервис в предполагаемый аналог автоматически.
