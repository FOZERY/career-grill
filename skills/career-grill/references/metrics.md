...
> `<сильный глагол>` + `<что изменил>` + `<технический механизм>` + `<метрика: было -> стало>`.
...
Формулировки `Improved API performance significantly` и `Improved API performance by 40%` недостаточны: не названы измеряемый путь, исходное/итоговое состояние и механизм. Сильная форма: `Cut frontier gRPC API p95 latency with concurrent downstream fan-out, reducing it from 680 ms to 214 ms.`
...
Для одной компании обычно достаточно 1-3 сильных измеримых результатов. Остальные достижения могут иметь качественный эффект, если он конкретен и защищаем. Не раздавай искусственную метрику каждому bullet.
...
Публичная, рассчитанная и оценочная метрики не становятся личным результатом кандидата автоматически. В `Паспорте для CV` factual claim и метрика получают отдельные атомарные статусы. `CV eligibility: allowed` возможен для измеренной метрики либо прозрачной оценки/расчёта, когда подтверждены исходные данные, метод, личный scope и приблизительная формулировка. Остальные уровни остаются контекстом или материалом для тренировки.
...
1. **Нет результата.** Описана только обязанность, технология или действие.
2. **Активность вместо эффекта.** `Wrote 30 endpoints` и `made 200 commits` показывают объём работы, а не ценность.
3. **Процент без baseline/result.** `Improved performance by 40%` нельзя проверить и понять.
4. **Круглое, абсолютное или чрезмерное число.** `1000%`, `300K RPS`, `100% coverage` требуют особенно сильного источника и контекста.
5. **Ложная точность.** `Reduced incidents by 35.56%` недопустимо без измерения, которое поддерживает такую точность.

При наличии red flag восстанови источник и границы измерения, ослабь точность, замени число диапазоном либо оставь честный качественный эффект.
