...
Метрика в легенде — это история изменения системы или продукта. Короткая строка будущего CV является производной от подробного рассказа: проблема → бизнес-значение → ответственность → решение → измерение → результат → защита.
...
**Как защищать.** Ответ «откуда знаю число», ограничения причинности, два уровня follow-up и что сделал бы сейчас.

**Короткая английская формула.** <будущий материал для CV; отдельный CV не создаётся>
```

## Глубина измерения

Не принимай ответ «посмотрели в Grafana». Доведи измерение до воспроизводимой цепочки:
...
В легенде сохрани короткий, конкретный ответ. Например:
...
| Ответ | Как сотрудник узнал число и как повторил бы измерение |
...
Покажи консервативный и верхний сценарии. Среднее за сутки не называй пиком: добавь обоснованный коэффициент пиковости. Проверь согласованность с публичным масштабом компании, числом инстансов, partitions, connection pools и возможностями хранилища.
...
Для естественных наборов положительных чисел, которые охватывают несколько порядков и не имеют искусственного минимума/максимума, первая значащая цифра часто распределена как:

`P(d) = log10(1 + 1/d)`, где `d ∈ {1,…,9}`.

Ориентиры: 1 — 30,1%; 2 — 17,6%; 3 — 12,5%; …; 9 — 4,6%.

Используй закон как диагностический сигнал для большого подходящего датасета. Несколько выбранных метрик, проценты, latency в узком диапазоне, SLA и ограниченные значения не образуют такой набор. Не подгоняй первые цифры под Benford: источники, расчёты и взаимная согласованность важнее.

## Финальная защита метрики

Подготовь ответ:

> «Проблема влияла на `<пользователь/бизнес>` так: `<эффект>`. Мы измеряли `<показатель>` в `<инструмент>` на уровне `<endpoint/сервис/продукт>`. Сравнивали `<период до>` и `<период после>` при `<условия>`. Было `<baseline>`, стало `<result>`. Я сделал `<действие>`; параллельно менялось `<другие факторы>`, поэтому описываю результат как `<причина/вклад>`».
